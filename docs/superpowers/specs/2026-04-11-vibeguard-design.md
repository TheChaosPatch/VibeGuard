# VibeGuard — Design Specification

**Project:** VibeGuard
**Acronym:** GUARD — **Global Unified AI Rules for Development**
**Public repository:** https://github.com/ehabhussein/VibeGuard
...
VibeGuard is an open-source **Model Context Protocol (MCP) server** that acts as a **high-to-low level architecture consultant** any LLM can consult *before* writing a function or class. Given a free-text intent and a language, the server returns curated, human-authored engineering guidance — principles, architectural placement, anti-patterns, library choices, and gotchas — drawn from a community-maintained corpus.

VibeGuard is **not** an LLM, not an agent, not a SAST tool, and not a ruleset. It is a deterministic content delivery server. Its intelligence lives in the *content*, which is written and reviewed by humans. The server's job is to retrieve the right piece of content for the right query.

### 1.3 Philosophy

VibeGuard is built on three non-negotiable commitments:
...
- Enforce anything. LLMs are free to ignore VibeGuard's output. Enforcement is a downstream concern (editor integrations, DeceptiCode-style review agents, CI).
...
The LLM is the only agent in the loop. It decides when to call `prep`, which archetypes from the returned list to drill into, and how to apply the guidance to the user's request. VibeGuard never reasons. It retrieves.
...
SecureCodingMcp.sln
├── src/
│   ├── VibeGuard.Mcp/                (executable, composition root)
│   │   ├── Program.cs
│   │   ├── Tools/
│   │   │   ├── PrepTool.cs
│   │   │   └── ConsultTool.cs
│   │   └── appsettings.json
│   └── VibeGuard.Content/            (class library, content domain)
│       ├── Archetype.cs
...
├── tests/
│   └── VibeGuard.Content.Tests/      (xUnit + FluentAssertions)
...
VibeGuard's threat model is unusual: the server ships with a community-authored content corpus, processes LLM-generated requests, and is often run on developer laptops or CI. The risks are:
...
**`csharp`, `python`, `c`, `go`.** Deliberately not JavaScript, not Java. VibeGuard targets backend and systems engineers.
...
5. A README exists explaining VibeGuard, containing the phrase "GUARD — Global Unified AI Rules for Development" verbatim.
6. Contribution guidelines (CONTRIBUTING.md) explain the archetype schema and how to add a new archetype or language file.
7. A `LICENSE` file exists at the repo root with the standard MIT license text, attributed to Ehab Hussein and the VibeGuard contributors.

---

## 8. Contribution model

VibeGuard is open source from day one. The contribution workflow is:
...
### 9.1 Unit tests (`VibeGuard.Content.Tests`)
...
- **`validate_code` tool.** VibeGuard is prescriptive, not reactive. The MCP tells the LLM how to build things, not how to grade what it already built.
