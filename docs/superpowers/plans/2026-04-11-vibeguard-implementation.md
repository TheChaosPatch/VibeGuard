...
**Architecture:** Three-project solution — `VibeGuard.Mcp` (executable, composition root, MCP tool handlers), `VibeGuard.Content` (domain, loading, indexing, services), `VibeGuard.Content.Tests` (xUnit + AwesomeAssertions). Content lives in a sibling `archetypes/` directory as plain markdown with YAML frontmatter. All content is eager-loaded and validated at startup; request-path is pure in-memory lookup.
...
**Design source:** `docs/superpowers/specs/2026-04-11-vibeguard-design.md` — read §3 (MCP contract), §4 (content schema), §5 (architecture), §6 (security) before starting.
...
- `AwesomeAssertions` — 9.4.0 (Apache-2.0 community fork of FluentAssertions 7.x; used instead of `FluentAssertions` because FluentAssertions 8+ became commercial under Xceed — incompatible with VibeGuard's MIT-for-free-adoption goal)
...
1. **Layered sub-namespaces — layer N depends only on layers 0..N−1.** VibeGuard's layers are:
   - Layer 0: **Domain** — `VibeGuard.Content` root (`SupportedLanguage`, `Archetype`, `PrinciplesFrontmatter`, `LanguageFrontmatter`, `LanguageFile`)
   - Layer 1: **Loading** — `VibeGuard.Content.Loading` (parsing, reading, path-traversal defense)
   - Layer 2: **Validation** — `VibeGuard.Content.Validation`
   - Layer 3: **Indexing** — `VibeGuard.Content.Indexing`
   - Layer 4: **Services** — `VibeGuard.Content.Services`
   - Layer 5: **MCP tool handlers** — `VibeGuard.Mcp.Tools`
   - Layer 6: **Composition root / host** — `VibeGuard.Mcp.Program`
...
5. **One DI extension method per library.** `VibeGuard.Content` exposes `AddVibeGuardContent(this IServiceCollection, string archetypesRoot)`; `VibeGuard.Mcp` composes it in `Program.cs`. No direct `services.AddSingleton<...>` calls in the host except what belongs to the host itself.
...
These principles are informed by the Cortex Refactor1 design doc Ehab shared on 2026-04-11 (see `reference_cortex_refactor1.md` in the project memory). VibeGuard deliberately does **not** adopt Cortex's "more granularity the better" rule — three projects at the right grain beats 99 projects at the wrong one.

**TDD discipline:** every production file in `VibeGuard.Content` is written test-first. Every task has explicit fail → implement → pass → commit steps. No batching of tests and implementation into a single commit.
...
- `src/VibeGuard.Content/VibeGuard.Content.csproj` — class library
- `src/VibeGuard.Content/SupportedLanguage.cs` — MVP language enum
- `src/VibeGuard.Content/PrinciplesFrontmatter.cs` — typed YAML model for `_principles.md`
- `src/VibeGuard.Content/LanguageFrontmatter.cs` — typed YAML model for `<lang>.md`
- `src/VibeGuard.Content/LanguageFile.cs` — record holding language-file frontmatter + body
- `src/VibeGuard.Content/Archetype.cs` — aggregate record (principles + all language files)
- `src/VibeGuard.Content/Loading/FrontmatterParser.cs` — splits frontmatter from body, strict YAML deserialize
- `src/VibeGuard.Content/Loading/FrontmatterParseException.cs` — thrown on malformed / unknown / missing fields
- `src/VibeGuard.Content/Loading/ArchetypeLoader.cs` — groups parsed files into `Archetype` aggregates
- `src/VibeGuard.Content/Validation/ArchetypeValidationException.cs` — thrown on body/budget violations
- `src/VibeGuard.Content/Validation/ArchetypeValidator.cs` — body sections, line budgets, code budgets
- `src/VibeGuard.Content/Loading/IArchetypeRepository.cs` — loader interface
- `src/VibeGuard.Content/Loading/FileSystemArchetypeRepository.cs` — reads disk, path-traversal defense
- `src/VibeGuard.Content/Indexing/PrepMatch.cs` — scored search hit DTO
- `src/VibeGuard.Content/Indexing/IArchetypeIndex.cs` — index interface
- `src/VibeGuard.Content/Indexing/KeywordArchetypeIndex.cs` — inverted + reverse-related index
- `src/VibeGuard.Content/Services/PrepResult.cs` — DTO
- `src/VibeGuard.Content/Services/IPrepService.cs`
- `src/VibeGuard.Content/Services/PrepService.cs` — tokenize, filter, score, top-8
- `src/VibeGuard.Content/Services/ConsultResult.cs` — DTO (normal + redirect + not-found variants)
- `src/VibeGuard.Content/Services/IConsultationService.cs`
- `src/VibeGuard.Content/Services/ConsultationService.cs` — composition, redirect, not-found
- `src/VibeGuard.Mcp/VibeGuard.Mcp.csproj` — executable
- `src/VibeGuard.Mcp/Program.cs` — generic host, DI, stdio MCP
- `src/VibeGuard.Mcp/Tools/PrepTool.cs` — `[McpServerTool]` for `prep`
- `src/VibeGuard.Mcp/Tools/ConsultTool.cs` — `[McpServerTool]` for `consult`
- `tests/VibeGuard.Content.Tests/VibeGuard.Content.Tests.csproj`
- `tests/VibeGuard.Content.Tests/FrontmatterParserTests.cs`
- `tests/VibeGuard.Content.Tests/ArchetypeLoaderTests.cs`
- `tests/VibeGuard.Content.Tests/ArchetypeValidatorTests.cs`
- `tests/VibeGuard.Content.Tests/FileSystemArchetypeRepositoryTests.cs`
- `tests/VibeGuard.Content.Tests/KeywordArchetypeIndexTests.cs`
- `tests/VibeGuard.Content.Tests/PrepServiceTests.cs`
- `tests/VibeGuard.Content.Tests/ConsultationServiceTests.cs`
- `tests/VibeGuard.Content.Tests/ContentCorpusSmokeTests.cs` — loads real `archetypes/`
...
- Create: `src/VibeGuard.Content/VibeGuard.Content.csproj`
- Create: `src/VibeGuard.Mcp/VibeGuard.Mcp.csproj`
- Create: `src/VibeGuard.Mcp/GlobalUsings.cs`
- Create: `tests/VibeGuard.Content.Tests/VibeGuard.Content.Tests.csproj`
- Create: `tests/VibeGuard.Content.Tests/GlobalUsings.cs`
...
<Project>
  ...
  <!--
    Central Package Management for VibeGuard.
...
Run:
```bash
dotnet new classlib -n VibeGuard.Content -o src/VibeGuard.Content --framework net10.0
dotnet sln SecureCodingMcp.slnx add src/VibeGuard.Content/VibeGuard.Content.csproj
```

Then overwrite `src/VibeGuard.Content/VibeGuard.Content.csproj` with:

```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <RootNamespace>VibeGuard.Content</RootNamespace>
    <AssemblyName>VibeGuard.Content</AssemblyName>
...
```bash
rm src/VibeGuard.Content/Class1.cs
```
...
Run:
```bash
dotnet new console -n VibeGuard.Mcp -o src/VibeGuard.Mcp --framework net10.0
dotnet sln SecureCodingMcp.slnx add src/VibeGuard.Mcp/VibeGuard.Mcp.csproj
```

Then overwrite `src/VibeGuard.Mcp/VibeGuard.Mcp.csproj` with:

```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <OutputType>Exe</OutputType>
    <RootNamespace>VibeGuard.Mcp</RootNamespace>
    <AssemblyName>vibeguard-mcp</AssemblyName>
    <IsPackable>false</IsPackable>
  </PropertyGroup>
  <ItemGroup>
    <ProjectReference Include="..\VibeGuard.Content\VibeGuard.Content.csproj" />
  </ItemGroup>
  <ItemGroup>
...
(`ModelContextProtocol` is the official MCP C# SDK — owner `ModelContextProtocol` on nuget.org, repo `modelcontextprotocol/csharp-sdk`, co-maintained with Microsoft. `AssemblyName` produces the `vibeguard-mcp` binary per spec §0 naming. Versions come from `Directory.Packages.props`.) Overwrite the default `Program.cs` with a stub we'll replace in Task 11:

```csharp
// Placeholder — replaced in Task 11 (composition root).
System.Console.Error.WriteLine("vibeguard-mcp: composition root not yet implemented");
return 1;
```

Also create `src/VibeGuard.Mcp/GlobalUsings.cs`:

```csharp
// Host-project global usings.
// VibeGuard.Mcp is a generic-host MCP server: DI, hosting, and logging
...
Run:
```bash
dotnet new xunit -n VibeGuard.Content.Tests -o tests/VibeGuard.Content.Tests --framework net10.0
dotnet sln SecureCodingMcp.slnx add tests/VibeGuard.Content.Tests/VibeGuard.Content.Tests.csproj
```

Then overwrite `tests/VibeGuard.Content.Tests/VibeGuard.Content.Tests.csproj` with:

```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <RootNamespace>VibeGuard.Content.Tests</RootNamespace>
    <AssemblyName>VibeGuard.Content.Tests</AssemblyName>
    <IsPackable>false</IsPackable>
    <IsTestProject>true</IsTestProject>
  </PropertyGroup>
  <ItemGroup>
    <ProjectReference Include="..\..\src\VibeGuard.Content\VibeGuard.Content.csproj" />
  </ItemGroup>
  <ItemGroup>
...
Every test-file code example in later tasks omits `using Xunit;` and `using AwesomeAssertions;` — they're global. Keep sub-namespace imports (`using VibeGuard.Content.Loading;` etc.) explicit per test file; globalizing every sub-namespace hides which subsystem a test actually targets.

Delete the `xunit`-template `UnitTest1.cs`:
```bash
rm tests/VibeGuard.Content.Tests/UnitTest1.cs
```
...
```bash
git add SecureCodingMcp.slnx Directory.Build.props Directory.Packages.props src/VibeGuard.Content/ src/VibeGuard.Mcp/ tests/VibeGuard.Content.Tests/
git commit -m "scaffold: three-project .slnx solution for VibeGuard MCP server
...
VibeGuard.Content class library, VibeGuard.Mcp executable, and
VibeGuard.Content.Tests xUnit project. All projects inherit strict
...
**Files:**
- Create: `src/VibeGuard.Content/SupportedLanguage.cs`
- Create: `src/VibeGuard.Content/PrinciplesFrontmatter.cs`
- Create: `src/VibeGuard.Content/LanguageFrontmatter.cs`
- Create: `src/VibeGuard.Content/LanguageFile.cs`
- Create: `src/VibeGuard.Content/Archetype.cs`
...
/// <summary>
/// The MVP language set for VibeGuard per design spec §7.1.
/// Deliberately excludes JavaScript and Java — VibeGuard targets
...
**Files:**
- Create: `src/VibeGuard.Content/Loading/FrontmatterParseException.cs`
- Create: `src/VibeGuard.Content/Loading/FrontmatterParser.cs`
- Create: `tests/VibeGuard.Content.Tests/FrontmatterParserTests.cs`
...
Run: `dotnet test tests/VibeGuard.Content.Tests/VibeGuard.Content.Tests.csproj --filter FullyQualifiedName~FrontmatterParserTests`
...
```bash
git add src/VibeGuard.Content/Loading/ tests/VibeGuard.Content.Tests/FrontmatterParserTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Loading/ArchetypeLoader.cs`
- Create: `tests/VibeGuard.Content.Tests/ArchetypeLoaderTests.cs`
...
```bash
git add src/VibeGuard.Content/Loading/ArchetypeLoader.cs tests/VibeGuard.Content.Tests/ArchetypeLoaderTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Validation/ArchetypeValidationException.cs`
- Create: `src/VibeGuard.Content/Validation/ArchetypeValidator.cs`
- Create: `tests/VibeGuard.Content.Tests/ArchetypeValidatorTests.cs`
...
```bash
git add src/VibeGuard.Content/Validation/ src/VibeGuard.Content/Loading/ArchetypeLoader.cs tests/VibeGuard.Content.Tests/ArchetypeValidatorTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Loading/IArchetypeRepository.cs`
- Create: `src/VibeGuard.Content/Loading/FileSystemArchetypeRepository.cs`
- Create: `tests/VibeGuard.Content.Tests/FileSystemArchetypeRepositoryTests.cs`
...
```bash
git add src/VibeGuard.Content/Loading/IArchetypeRepository.cs src/VibeGuard.Content/Loading/FileSystemArchetypeRepository.cs tests/VibeGuard.Content.Tests/FileSystemArchetypeRepositoryTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Indexing/PrepMatch.cs`
- Create: `src/VibeGuard.Content/Indexing/IArchetypeIndex.cs`
- Create: `src/VibeGuard.Content/Indexing/KeywordArchetypeIndex.cs`
- Create: `tests/VibeGuard.Content.Tests/KeywordArchetypeIndexTests.cs`
...
```bash
git add src/VibeGuard.Content/Indexing/ tests/VibeGuard.Content.Tests/KeywordArchetypeIndexTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Services/PrepResult.cs`
- Create: `src/VibeGuard.Content/Services/IPrepService.cs`
- Create: `src/VibeGuard.Content/Services/PrepService.cs`
- Create: `tests/VibeGuard.Content.Tests/PrepServiceTests.cs`
...
```bash
git add src/VibeGuard.Content/Services/PrepResult.cs src/VibeGuard.Content/Services/IPrepService.cs src/VibeGuard.Content/Services/PrepService.cs tests/VibeGuard.Content.Tests/PrepServiceTests.cs
...
**Files:**
- Create: `src/VibeGuard.Content/Services/ConsultResult.cs`
- Create: `src/VibeGuard.Content/Services/IConsultationService.cs`
- Create: `src/VibeGuard.Content/Services/ConsultationService.cs`
- Create: `tests/VibeGuard.Content.Tests/ConsultationServiceTests.cs`
...
```bash
git add src/VibeGuard.Content/Services/ConsultResult.cs src/VibeGuard.Content/Services/IConsultationService.cs src/VibeGuard.Content/Services/ConsultationService.cs tests/VibeGuard.Content.Tests/ConsultationServiceTests.cs
...
**Files:**
- Create: `src/VibeGuard.Mcp/Tools/PrepTool.cs`
- Create: `src/VibeGuard.Mcp/Tools/ConsultTool.cs`
...
Note: `VibeGuard.Mcp` still has the stub `Program.cs` from Task 1 — the build will succeed because the stub is a valid top-level-statements program. The tool types will be picked up automatically by `.WithToolsFromAssembly()` once we write the real composition root in Task 11.
...
```bash
git add src/VibeGuard.Mcp/Tools/
...
Then edit `src/VibeGuard.Mcp/VibeGuard.Mcp.csproj` and add the appsettings file to the output:
...
Overwrite `src/VibeGuard.Mcp/Program.cs`:
...
catch (Exception ex)
{
    ...
    logger.LogCritical(ex, "VibeGuard failed to load archetype corpus from {Root}", archetypesRoot);
    return 1;
}
...
Run:
```bash
set VIBEGUARD_ARCHETYPES_ROOT=F:\repositories\SecureCodingMcp\nonexistent-root
dotnet run --project src/VibeGuard.Mcp --no-build
```
...
```bash
VIBEGUARD_ARCHETYPES_ROOT=F:/repositories/SecureCodingMcp/nonexistent-root dotnet run --project src/VibeGuard.Mcp --no-build
```

Then unset the variable so later steps aren't affected:
```bash
unset VIBEGUARD_ARCHETYPES_ROOT
```

- [ ] **Step 5: Commit**

```bash
git add src/VibeGuard.Mcp/Program.cs src/VibeGuard.Mcp/appsettings.json src/VibeGuard.Mcp/VibeGuard.Mcp.csproj
...
root from VIBEGUARD_ARCHETYPES_ROOT / appsettings.json / default,
...
dotnet run --project src/VibeGuard.Mcp --no-build < /dev/null
```
...
On Windows CMD use `echo. | dotnet run --project src/VibeGuard.Mcp --no-build` instead.
...
**Files:**
- Create: `tests/VibeGuard.Content.Tests/ContentCorpusSmokeTests.cs`
...
The test needs to find the archetypes directory at runtime. Since `dotnet test` runs from `tests/VibeGuard.Content.Tests/bin/Debug/net10.0`, we walk up until we find `SecureCodingMcp.slnx` and then resolve `archetypes/` beside it.
...
```bash
git add tests/VibeGuard.Content.Tests/ContentCorpusSmokeTests.cs
...
VibeGuard is an open-source Model Context Protocol (MCP) server that gives any LLM a high-to-low-level architecture consultant it can call before writing a function or class. It ships human-authored engineering guidance — principles, architectural placement, anti-patterns, library choices, and language-specific gotchas — organized into focused **archetypes** that the LLM retrieves via two tools: `prep` and `consult`.

VibeGuard is **not** a static analyzer, not a ruleset, not an LLM, and not an agent. It is a deterministic content-delivery server. The intelligence lives in the content, which is written and reviewed by humans through a PR workflow.

## Why

LLMs generate code that works but defaults to the insecure and architecturally poor path: MD5-hashed passwords, SQL string concatenation, god-functions that mix HTTP and persistence concerns, inconsistent error handling. The fix isn't to bolt more checks onto generated code — it's to give the LLM a place to consult *before* it writes the function. That's what VibeGuard does.
...
```bash
VIBEGUARD_ARCHETYPES_ROOT=/path/to/archetypes dotnet run --project src/VibeGuard.Mcp
```

### Wire into Claude Desktop or Claude Code

Add an MCP server entry pointing at the compiled `vibeguard-mcp` binary. See your client's MCP configuration docs; the exact shape varies by client.
...
├── src/
│   ├── VibeGuard.Mcp/        (executable, composition root, MCP tool handlers)
│   └── VibeGuard.Content/    (domain, loading, indexing, services)
├── tests/
│   └── VibeGuard.Content.Tests/
├── archetypes/               (the content corpus)
└── docs/superpowers/specs/2026-04-11-vibeguard-design.md
```
...
VibeGuard is open-sourced by Ehab Hussein and the VibeGuard contributors. The goal is broad, friction-free adoption across the LLM ecosystem — Claude, GPT, Cursor, and anything else that speaks MCP.
```
...
README introduces VibeGuard, states the required GUARD expansion
...
dotnet run --project src/VibeGuard.Mcp -c Release --no-build < /dev/null
```
...
- §9.4 TDD discipline → every production file in VibeGuard.Content is written test-first (Tasks 3–9)
...
5. **Push to `https://github.com/ehabhussein/VibeGuard`.** Remote is already configured but nothing has been pushed. Do not push without explicit user authorization.
